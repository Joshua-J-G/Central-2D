...
public class Camera 
{
	...
	public void update(GameContainer gc, GameManger gm, float dt)
	{
		...
		offX = targetX;
		offY = targetY;
		//offX -= dt * (int)(offX - targetX) * CSpeed;
		//offY -= dt * (int)(offY - targetY) * CSpeed;
		
			if(offX < 0) offX = 0;
			if(offY < 0) offY = 0;
			if(offX + gc.getWidth() > gm.getLevelW() * GameManger.TS) offX = gm.getLevelW() * GameManger.TS - gc.getWidth();
			if(offY + gc.getHeight() > gm.getLevelH() * GameManger.TS) offY = gm.getLevelH() * GameManger.TS - gc.getHeight();
